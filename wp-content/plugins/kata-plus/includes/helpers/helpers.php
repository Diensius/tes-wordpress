...
if ( ! class_exists( 'Kata_Plus_Helpers' ) ) {
	...
	class Kata_Plus_Helpers {
		...
		public function actions() {
			...
			add_action( 'admin_init', [$this, 'disable_redirection'], 1 );
		}
		...
		public static function get_the_author_social_networks() {
			...
			if( $facebook || $twitter || $linkedin || $instagram ) {
				...
				if( $facebook ) {
					?>
					<li> <a href="<?php echo esc_url( $facebook ); ?>"><?php echo Kata_Plus_Helpers::get_icon( '', 'font-awesome/facebook', '', '' ); ?></a> </li>
					<?php
				}
				if( $twitter ) {
					?>
					<li> <a href="<?php echo esc_url( $twitter ); ?>"><?php echo Kata_Plus_Helpers::get_icon( '', 'font-awesome/twitter', '', '' ); ?></a> </li>
					<?php
				}
				if( $linkedin ) {
					?>
					<li> <a href="<?php echo esc_url( $linkedin ); ?>"><?php echo Kata_Plus_Helpers::get_icon( '', 'font-awesome/linkedin', '', '' ); ?></a> </li>
					<?php
				}
				if( $instagram ) {
					?>
					<li> <a href="<?php echo esc_url( $instagram ); ?>"><?php echo Kata_Plus_Helpers::get_icon( '', 'font-awesome/instagram', '', '' ); ?></a> </li>
					<?php
				}
				echo '</ul>';
			}
		 ...
		 * @since   1.0.0
		 */
		public static function post_format_icon( $icon = [] ) {
			$icon = [
