...
(function ($) {
    ...
    $(".demo-categories").find('select[name="demo-categories"]').on("change", function () {
        $("#kata-importer-search-styles").remove();
        $( '<style id="kata-importer-search-styles">.kata-importer[website-type] {display:none;}.kata-importer[website-type*="' + this.value + '"]{display:inline-block;}</style>' ).appendTo("head");
    });
    ...
    $(".kata-demotypes") .find(".demotypeitem") .on("click", function () {
            var $this = $(this),
     ...
     * @since	1.0.0
     */
    $(".kata-demo-importer-search-box").find('input[type="text"]').on("input", function (el) {
    ...
    function StepManager() {
        setTimeout(function () {
            if (!$(".kata-required-plugin:not(.active)").length) {
                ...
                var currnet_location = window.location.search.indexOf('kata-plus-fast-mode') >= 0 ? 'fastmode' : 'importer'
    ...
    $(document).ajaxComplete(function (event, xhr, options) {
        if (options.name == "ImporterBuildSteps") {
            ...
            $(".kata-import-demo-btn").on("click", function (e) {
                ...
                e.preventDefault();
                $parent.find(".kata-import-content-wrap").find(".kata-checkbox-input").attr("disabled","disabled");
                $wrap.find(".kata-import-demo-btn").addClass("disabled");
    ...
    function ImporterSteps() {
        StepManager();
        $(".kata-lightbox").find(".kt-importer-step").on("click", function () {
                var $this = $(this),
                ...
                var currnet_location = window.location.search.indexOf('kata-plus-fast-mode') >= 0 ? 'fastmode' : 'importer'
                if ( currnet_location != 'fastmode' ) {
                    $wrap.find(".kata-lightbox-content").attr("style", "transform: translateX(-" + width * step + "px)");
                }
            });
    }
    ...
    $(".kata-importer")
        ...
        .on("click", function () {
            var $this = $(this),
                ...
                currnet_location = window.location.search.indexOf('kata-plus-fast-mode') >= 0 ? 'fastmode' : 'importer',
