...
(function ($) {
     ...
     */
     $('.kt-fst-get-info').find('#site-facebook').on('input', function (el) {
        clearTimeout(timer);
        timer = setTimeout(function () {
            var $this = $(this),
                val = jQuery(el.target).val(),
     ...
     */
     $('.kt-fst-get-info').find('#site-twitter').on('input', function (el) {
        clearTimeout(timer);
        timer = setTimeout(function () {
            var $this = $(this),
                val = jQuery(el.target).val(),
     ...
     */
     $('.kt-fst-get-info').find('#site-linkedin').on('input', function (el) {
        clearTimeout(timer);
        timer = setTimeout(function () {
            var $this = $(this),
                val = jQuery(el.target).val(),
     ...
     */
     $('.kt-fst-get-info').find('#site-instagram').on('input', function (el) {
        clearTimeout(timer);
        timer = setTimeout(function () {
            var $this = $(this),
                val = jQuery(el.target).val(),
     ...
     * Select Demo
     */
    $(document).on('click', '.kata-demp-wrapper, .kata-importer-name, .kata-importer-screenshot', function(e) {
        var $this = $(this),
            $item = '',
    ...
    $(document).on('click', '.next-step.ready', function(e) {
        ...
        if ( $(this).hasClass('ready') ) {
            $('.kata-importer-wrapper:not(.fast-mode-preview) [data-key="' + $(this).attr('data-key') + '"]').find('.kata-btn-importer').trigger('click');
        }
    ...
    $(document).ajaxSuccess(function (event, xhr, settings) {
        if ( settings.data ) {
            if ( settings.data.indexOf("ImportDone") >= 0 ) {
                ...
                window.location.replace(href + '?page=kata-plus-fast-mode&step=6');
            }
        }
    });
    ...
    $(document).ajaxSuccess(function (event, xhr, settings) {
        if ('ImporterBuildSteps' == settings.name) {
            setTimeout(function () {
                ...
                jQuery('.kata-col-import-demo').find('.tasks').append(`<li class="kata-importer-task-menus kata-import-active" data-action="plugins" data-status="pending">Installing Plugins</li>`);
                ...
                $('.kata-checkbox-input').each(function (index, element) {
                    ...
                    if ( typeof data !== 'undefined' ) {
                        var text = data == 'builders' ? 'Header, Footer...': data;
                        $('.kata-col-import-demo').find('.tasks').append(`<li class="kata-importer-task-menus kata-import-pending" data-action="${data}" data-status="pending">Import ${text}</li>`);
                    }
                });
            }, 500);
        }  
        ...
        if ( settings.url ) {
            if ( settings.url.indexOf("kata_plus_plugin_actions") >= 0 ) {
                ...
                if ( plugins_size == plugin_size_compare ) {
                    setTimeout(function () {
                        $('.kata-btn-install-plugins').trigger('click');
                        $('.kata-importer-task-menus[data-action="plugins"]').removeClass('kata-import-active');
                    }, 500);
                    setTimeout(function () {
                        $('.kata-importer-task-menus[data-action="plugins"]').attr('class','kata-importer-task-menus kata-import-done');
                    }, 1000);
                }
                setTimeout(function () {
     ...
     * @since	1.0.0
     */
    $(".fast-mode-search-demo").find('.site-search').on("input", function (el) {
        clearTimeout(timer); //clear any running timeout on key up
        timer = setTimeout(function () {
            var searchValue = jQuery(el.target).val();
            $("#kata-importer-search-styles").remove();
            if (searchValue) {
                $( '<style id="kata-importer-search-styles">.kata-importer[demo-name] {display:none;}.kata-importer[demo-name*="' + searchValue + '"]{display:inline-block;}</style>' ).appendTo("head");
            }
        }, 250);
    });
